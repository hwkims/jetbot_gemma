...
import json
import logging
from jetbot import Robot, Camera, bgr8_to_jpeg  # Jetbot libraries
import base64
import time

# --- OpenCV Handling ---
try:
    import cv2  # Try to import OpenCV
    OPENCV_AVAILABLE = True
    logger.info("OpenCV (cv2) found.")
except ImportError:
    cv2 = None  # Set cv2 to None if import fails
    OPENCV_AVAILABLE = False
    logger.warning("OpenCV (cv2) not found.  Image streaming will be slower.")
...
async def handle_command(command, parameters=None):
    ...
    try:
        duration = parameters.get("duration", 1.0)  # Get duration, default 1.0
...
async def websocket_handler(websocket, path):
    ...
    async def send_image_stream():
        ...
        try:
            while True:
                ...
                if OPENCV_AVAILABLE: # Use OpenCV if available
                    _, encoded_image = cv2.imencode('.jpg', frame)
                    image_base64 = base64.b64encode(encoded_image).decode('utf-8')

                else: # Fallback to bgr8_to_jpeg if cv2 is missing
    ...
    if hasattr(asyncio, 'create_task'): #For python 3.7+
    ...
    while True:
        try:
            ...
            if "Address already in use" in str(e):
                logger.error(
                    f"Address already in use: {e}. Retrying in {RECONNECT_DELAY} seconds..."
                )
